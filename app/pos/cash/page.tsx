...
export default function PosCashPage() {
  ...
  const { data: sessions, mutate } = useSWR(posId ? `/api/pos/cash?posId=${posId}` : null, fetcher)
  ...
  async function handleOpen() {
    const res = await fetch("/api/pos/cash", {
      ...
      body: JSON.stringify({ pointOfSaleId: posId, action: "open", openingAmount: Number(openAmount) || 0 }),
    })
    ...
    <div>
      ...
      {!posId ? (
        ...
        <div className="flex flex-col gap-6">
          {openSession ? (
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                  <div>
                    ...
                    <p className="text-xl font-bold text-card-foreground">{Number(openSession.opening_amount).toLocaleString()} CDF</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Ventes</p>
                    <p className="text-xl font-bold text-primary">{Number(openSession.total_sales).toLocaleString()} CDF</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Especes</p>
                    <p className="text-xl font-bold text-card-foreground">{Number(openSession.total_cash).toLocaleString()} CDF</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Mobile Money</p>
                    <p className="text-xl font-bold text-card-foreground">{Number(openSession.total_mobile).toLocaleString()} CDF</p>
                  </div>
                </div>
                <p className="mt-3 text-xs text-muted-foreground">
                  Ouverte par: {openSession.opened_by_name} le {new Date(openSession.opened_at).toLocaleString("fr-FR")}
                </p>
                <Button
                  onClick={() => {
                    setClosingSession(openSession.id)
                    setCloseAmount(String(Number(openSession.opening_amount) + Number(openSession.total_cash)))
                  }}
            ...
            <Card className="border-border bg-card">
              <CardContent className="p-6">
                <h3 className="mb-3 flex items-center gap-2 font-medium text-card-foreground">
                  <DollarSign className="h-5 w-5 text-primary" /> Ouvrir la caisse
                </h3>
          ...
          <Card className="border-border bg-card">
            <CardHeader><CardTitle className="text-base text-card-foreground">Historique</CardTitle></CardHeader>
            <CardContent className="p-0">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {(sessions || []).map((s: { id: string; status: string; opened_by_name: string; opening_amount: number; total_sales: number; closing_amount: number; opened_at: string }) => (
                      <tr key={s.id} className="border-b border-border/50">
                        ...
                        <td className="px-4 py-3 font-medium text-primary">{Number(s.total_sales).toLocaleString()}</td>
