...
import { AlertTriangle, Package, ClipboardList, RefreshCw } from "lucide-react"
...
export default function WarehouseStockPage() {
  ...
  const filtered = (stock || []).filter((s: { product_name: string; category_name: string }) =>
    s.product_name.toLowerCase().includes(search.toLowerCase()) || (s.category_name || "").toLowerCase().includes(search.toLowerCase())
  )
      ...
      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-3">
          <Input
            ...
            className="w-[200px] border-border bg-secondary text-foreground"
          />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="px-4 py-3 text-left font-medium text-muted-foreground">Theorique</th>
                  <th className="px-4 py-3 text-left font-medium text-muted-foreground">Reel</th>
              ...
              <tbody>
                {filtered.map((s: { product_id: string; product_name: string; category_name: string; storage_unit: string; theoretical_qty: number; real_qty: number; low_stock_threshold: number }) => {
                  ...
                  return (
                    <tr key={s.product_id} className="border-b border-border/50">
                      ...
                      <td className="px-4 py-3">
                        {diff !== 0 ? (
                          <span className={diff > 0 ? "text-emerald-400" : "text-destructive"}>{diff > 0 ? "+" : ""}{diff}</span>
                        ) : (
