...
const navItems = [
  { href: "/pos", label: "Commandes", icon: ShoppingCart },
  { href: "/pos/tables", label: "Tables", icon: Store },
  { href: "/pos/billing", label: "Facturation", icon: Receipt },
  { href: "/pos/cash", label: "Caisse", icon: DollarSign },
  { href: "/pos/menu", label: "Menu", icon: UtensilsCrossed },
]
    ...
    <aside className="flex h-screen w-56 flex-col border-r border-sidebar-border bg-sidebar">
      <div className="flex items-center gap-3 border-b border-sidebar-border px-4 py-4">
        ...
        <div>
          ...
          <p className="text-[9px] tracking-widest text-primary uppercase">Point de Vente</p>
        </div>
      </div>
      ...
      {session?.user && (
        <div className="border-b border-sidebar-border px-4 py-3">
          ...
          <p className="text-[10px] capitalize text-muted-foreground">{session.user.role === "cashier" ? "Caissier(e)" : "Serveur(se)"}</p>
        </div>
      )}
